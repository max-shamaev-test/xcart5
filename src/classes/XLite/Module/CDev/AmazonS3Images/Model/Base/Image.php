...
use XLite\Controller\Admin\EventTask;
use XLite\Core\Queue\Scheduler\SchedulerService;
use XLite\Module\CDev\AmazonS3Images\Core\Job\CopyFileToS3;
use XLite\Module\CDev\AmazonS3Images\Core\Job\ResizeIconS3;
...
abstract class Image extends \XLite\Model\Base\Image implements \XLite\Base\IDecorator
{
    ...
    public function loadFromLocalFile($path, $basename = null, $makeUnique = false)
    {
        if (!$this->isS3Forbid() && $this->getS3() && !$this->isTemporaryFile()) {
            ...
            // Now ready to uploading to the s3 storage
            $path = $this->getStoragePath();

            $job = new CopyFileToS3($this, $path, $basename ?: basename($path));

            $useRequestEndScheduler = !(\XLite::getController() instanceof EventTask);
            SchedulerService::createDefaultJobScheduler(null, $useRequestEndScheduler)->schedule($job);
    ...
    public function loadFromLocalFileAsync($path, $basename)
    {
        $result = false;
    ...
    protected function resizeIcon($width, $height, $path)
    {
        ...
        if ($this->isUseS3Icons()) {
            if ($this->isManaged()) {
                $job = new ResizeIconS3($this, $width, $height, $path);
                SchedulerService::createDefaultJobScheduler()->schedule($job);
            } else {
                $this->resizeIconAsync($width, $height, $path);
            }
    ...
    public function resizeIconAsync($width, $height, $path)
    {
        $result = null;

        $operator = new \XLite\Core\ImageOperator($this);
        list($newWidth, $newHeight, $r) = $operator->resize($width, $height);
        ...
        if (false !== $r) {
            ...
            if ($this->getS3()->write($path, $operator->getImage()->getBody(), $headers)) {
                $icons = $this->getS3icons();
                $icons[$path] = true;
     ...
     * @param array $s3icons
     */
    public function setS3icons($s3icons)
    {
        $this->s3icons = $s3icons;
    }

    /**
     * @return array
     */
    public function getS3icons()
    {
        return $this->s3icons;
    }
}
